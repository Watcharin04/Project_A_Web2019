var App = function() {
	...
	var uiInit = function() {
        ...
        if (element.is('li')) {
            if(element.hasClass('nav-item-has-subnav')){
    ...
    // Selected Data table functionality
    var uiHelperTableSelected = function() {

        jQuery(".site-action-toggle", ".site-action").on("click", function (e) {
            var $selected = jQuery("#ajax_datatable input:checkbox:checked").map(function(){
                return jQuery(this).val();
            }).get();
            if($selected.length == 0){
                jQuery("#slidepanel-show").trigger('click'), e.stopPropagation();
            } else{
                jQuery('#ajax_datatable  input:checkbox').removeAttr('checked');
                jQuery('#ajax_datatable tr').removeClass('active');
            }
        }),
        jQuery('[data-action="trash"]', ".site-action").on("click", function () {
            var $selected = jQuery("#ajax_datatable input:checkbox:checked").map(function(){
                return jQuery(this).val();
            }).get();
            ...
            bootbox.dialog({
                ...
                buttons: {
                    success: {
                        label: "Delete", className: "btn-danger danger-loader", callback: function () {
                            var data = {
                                'action': 'deleteusers',
                                'list': ser_id
                            };
                            jQuery('.danger-loader').addClass('bookme-progress');
                            var actionBtn = jQuery(".site-action").actionBtn().data("actionBtn");
                            jQuery.post(ajaxurl, data, function (response) {
                                bootbox.hideAll();
                                if (response == 1) {
                                    alertify.success("Services Deleted");
                                    jQuery('.danger-loader').removeClass('bookme-progress');
                                    actionBtn.hide();
                                    jQuery.each(ser_id, function (index, item) {
                                        jQuery('#ajax_datatable input:checkbox:checked').parents('tr').remove();
                                    });

                                } else {
                                    alertify.error("Problem in delete user.");
                                    jQuery('.danger-loader').removeClass('bookme-progress');
                                    actionBtn.hide();
                                }
                            });
	...
	var uiHelperTableToolsCheckable = function() {
		...
		jQuery( 'tbody > tr', $table ).click( function(e) {
			if ( e.target.type !== 'checkbox' && e.target.type !== 'button' && e.target.tagName.toLowerCase() !== 'a' && e.target.tagName.toLowerCase() !== 'i' && !jQuery( e.target ).parent( 'label' ).length ) {
